<template>
  ...
  >
    ...
    <nav
      class="fixed w-full h17 left-0 bottom-0 bg-white/90 flex justify-evenly gap-x2 px2 [&>a]:(flex-grow w0 flex flex-col justify-center items-center gap1 text-xs text-gray-700)"
    >
      <a href="">
  ...
  <main class="mt4 bg-white">
    <!-- 主視覺 -->
    <div class="container pt8 flex flex-col items-center gap-y4 relative">
      <p
        class="font-['Mantou_Sans'] text-13 leading-[1.05] tracking-0.5 text-center bg-clip-text text-transparent bg-gradient-to-r from-#E5793B to-#FF4185"
      >
        台灣的明天<br />喵先鋪路
      </p>

      <div class="flex flex-col gap-y-4 items-center md:flex-row">
        <p class="text-5 font-700 text-white px4 py3 bg-gray-700 rounded-4">2024 立委參選人</p>
        <p
          class="flex items-center gap-x4 px4 py3 bg-white/90 rounded-4 shadow-[0_4px_12px_0_#A3B4CB33] text-7 font-700 text-primary before:(content-[attr(data-num)] text-6.25 font-900 text-white bg-gradient w10 h10 rounded-full flex justify-center items-center bg-gradient-to-r from-#E5793B to-#FF4185)"
          data-num="3"
        >
          喵立翰 Miao LiHan
        </p>
    ...
    <!-- 跑馬燈 -->
    <!-- TODO: 改字型 & RWD -->
    <ul class="flex overflow-hidden bg-primary py2.5">
      <li v-for="n in 2" class="marquee whitespace-nowrap text-white text-10 [&>span]:px-2">
        <span>為喵星人，護台灣！</span>
